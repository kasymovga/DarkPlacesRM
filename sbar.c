...
void Sbar_ShowFPS(void)
{
	...
	if (showtime.integer)
	{
		...
		Sys_TimeString(showtime_format.string, timestring, sizeof(timestring));
	...
	{
		Cvar_LockThreadMutex();
		Sys_TimeString(showdate_format.string, datestring, sizeof(datestring));
		Cvar_UnlockThreadMutex();
		fps_strings++;
	}
	if (showblur.integer)
...
void Sbar_DeathmatchOverlay (void)
{
	...
	if (cl.last_ping_request < realtime - 2 && cls.netcon)
	{
		...
		else if (cls.protocol == PROTOCOL_QUAKE || cls.protocol == PROTOCOL_QUAKEDP || cls.protocol == PROTOCOL_DARKPLACES1 || cls.protocol == PROTOCOL_DARKPLACES2 || cls.protocol == PROTOCOL_DARKPLACES3 || cls.protocol == PROTOCOL_DARKPLACES4 || cls.protocol == PROTOCOL_DARKPLACES5 || cls.protocol == PROTOCOL_DARKPLACES6/* || cls.protocol == PROTOCOL_DARKPLACES7*/)
		{
...
{
	int i, numlines, range_begin, range_end;
