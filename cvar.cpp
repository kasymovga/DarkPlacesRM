...
#include <map>
#include <string>
std::map<std::string, cvar_t*> cvar_hashtable;
typedef std::map<std::string, cvar_t*>::iterator hashtable_iter_t;
...
{
    hashtable_iter_t res = cvar_hashtable.find(var_name);
    if(cvar_hashtable.end() == res)
        return NULL;
    else
        return res->second;
}
...
static cvar_t *Cvar_FindVarLink (const char *var_name, cvar_t **parent, cvar_t ***link, cvar_t **prev_alpha)
{
	...
	for (;var;var = var->nextonhashchain)
	{
...
// alphanumerical order
    ...
    cvar_hashtable[variable->name] = variable;
}
...
void Cvar_RestoreInitState(void)
{
	...
	for (cp = &cvar_vars;(c = *cp);)
	{
		...
		else
		{
			...
			for (cp2 = &cvar_hashtable[c->name];(c2 = *cp2);)
			{
				if (c2 == c)
				{
