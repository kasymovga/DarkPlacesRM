...
cvar_t snd_streaming_length = { CVAR_SAVE, "snd_streaming_length", "3", "decompress sounds completely if they are less than this play time when snd_streaming is 1"};
		...
		if (intensity > 0)
		{
			...
			if (snd_spatialization_occlusion.integer)
			{
				...
				if(snd_spatialization_occlusion.integer & 2)
					if(!occluded)
						if(cl.worldmodel && cl.worldmodel->brush.TraceLineOfSight && !cl.worldmodel->brush.TraceLineOfSight(cl.worldmodel, listener_origin, ch->origin, false))
							occluded = true;
			}
			if(occluded)
				intensity *= 0.5f;
