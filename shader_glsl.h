...
"#endif\n",
"\n",
"#if defined(GLSL130) || defined(GLSL140) || defined(GL_ES) || __VERSION__ >= 130\n",
"precision highp float;\n",
"#endif\n",
"#if defined(GLSL130) || defined(GLSL140) || __VERSION__ >= 130\n",
"# ifdef VERTEX_SHADER\n",
...
"#  define dp_FragColor2 gl_FragData[1]\n",
"# endif\n",
...
"\n",
"#ifdef USESPECULAR\n",
"	dp_FragColor = vec4((DeferredColor_Ambient + DeferredColor_Diffuse * diffuse) * fade, 1.0);\n",
"	dp_FragColor2 = vec4(DeferredColor_Specular * (specular * fade), 1.0);\n",
"# ifdef USECUBEFILTER\n",
"	vec3 cubecolor = dp_textureCube(Texture_Cube, CubeVector).rgb;\n",
"	dp_FragColor.rgb *= cubecolor;\n",
"	dp_FragColor2.rgb *= cubecolor;\n",
"# endif\n",
"#else\n",
"# ifdef USEDIFFUSE\n",
"	dp_FragColor = vec4((DeferredColor_Ambient + DeferredColor_Diffuse * diffuse) * fade, 1.0);\n",
"# else\n",
"	dp_FragColor = vec4(DeferredColor_Ambient * fade, 1.0);\n",
"# endif\n",
"# ifdef USECUBEFILTER\n",
"	vec3 cubecolor = dp_textureCube(Texture_Cube, CubeVector).rgb;\n",
"	dp_FragColor.rgb *= cubecolor;\n",
...
"# endif\n",
"		normal += dp_texture2D(Texture_Normal, (TexCoord + vec2(-0.06, -0.09)*ClientTime*NormalmapScrollBlend.x)*NormalmapScrollBlend.y*0.75).rgb;\n",
"		vec2 ScreenTexCoordForReflection = SafeScreenTexCoord + vec3(normalize(cast_myhalf3(normal))).xy * DistortScaleRefractReflect.zw;\n",
"	#else\n",
"		vec2 ScreenTexCoordForReflection = SafeScreenTexCoord + vec3(normalize(cast_myhalf3(offsetMappedTexture2D(Texture_Normal)) - cast_myhalf3(0.5))).xy * DistortScaleRefractReflect.zw;\n",
"	#endif\n",
"	// FIXME temporary hack to detect the case that the reflection\n",
"	// gets blackened at edges due to leaving the area that contains actual\n",
"	// content.\n",
"	// Remove this 'ack once we have a better way to stop this thing from\n",
"	// 'appening.\n",
"	float f = min(1.0, length(dp_texture2D(Texture_Reflection, ScreenTexCoordForReflection + vec2(0.01, 0.01)).rgb) / 0.05);\n",
"	f      *= min(1.0, length(dp_texture2D(Texture_Reflection, ScreenTexCoordForReflection + vec2(0.01, -0.01)).rgb) / 0.05);\n",
"	f      *= min(1.0, length(dp_texture2D(Texture_Reflection, ScreenTexCoordForReflection + vec2(-0.01, 0.01)).rgb) / 0.05);\n",
"	f      *= min(1.0, length(dp_texture2D(Texture_Reflection, ScreenTexCoordForReflection + vec2(-0.01, -0.01)).rgb) / 0.05);\n",
"	ScreenTexCoordForReflection = mix(SafeScreenTexCoord, ScreenTexCoordForReflection, f);\n",
"	color.rgb = mix(color.rgb, cast_myhalf3(dp_texture2D(Texture_Reflection, ScreenTexCoordForReflection)) * ReflectColor.rgb, ReflectColor.a);\n",
"#endif\n",
"#ifdef USEOCCLUDE\n",
